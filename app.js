...
const editionLabel = record => record.kind === 'special' ? '专题精读' : record.kind === 'backfill' ? '补查日报' : '研究资讯日报';
const fullLabel = record => record.kind === 'special' ? '完整专题' : '完整日报';
...
function navigation(active, date) {
  ...
  const links = data.digests.map((d,i) => `<a class="archive-link ${d.date === date ? 'active' : ''}" href="#daily/${d.date}" ${d.date === date ? 'aria-current="page"' : ''}>${esc(dateText(d.date))}<small>${i === 0 ? '最近归档 · ' : ''}${editionLabel(d)}</small></a>`).join('');
...
function renderDaily(date, domain = '') {
  ...
  main.innerHTML = head(`${editionLabel(digest)} / ${dateText(digest.date)}`, digest.kind === 'special' ? digest.title : '本期研究，值得继续追问', `${digest.items.length} 条研究线索 · 约 ${digest.readMinutes} 分钟 · ${editionLabel(digest)}`, issue) +
    `<div class="coverage-note">${esc(text(digest.intro))}<details class="coverage-details"><summary>查看本次覆盖情况</summary><p>${esc(text(digest.coverageNote))}</p></details></div><div class="reading-actions"><span class="subline">${domain ? `${esc(domainName(domain))} · ${filtered.length} 条` : '保留原始来源、研究条件与局限'}</span><a class="text-link" href="#report/${digest.date}">阅读${fullLabel(digest)} ↗</a></div><div class="filter-row" aria-label="按研究领域浏览"><a class="filter ${!domain?'active':''}" href="#daily/${digest.date}" ${!domain?'aria-current="true"':''}>全部</a>${domains.map(d => `<a class="filter ${domain===d?'active':''}" href="#daily/${digest.date}?domain=${encodeURIComponent(d)}" ${domain===d?'aria-current="true"':''}>${esc(domainName(d))}</a>`).join('')}</div>` +
